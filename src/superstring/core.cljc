(ns superstring.core
  (:require [clojure.string :as str]
            #?(:clj [superstring.macros :refer [alias-ns defalias]])
            [clojure.set :as set])
...
(defn ^String upper-case?
  ...
  {:pre [(string? s)]
   :post [(or (nil? %) (string? %))]}
  (when (reduce
         (fn [acc c] (and acc (= c #?(:cljs (.toLocaleUpperCase c)
...
(defn lower-case?
  ...
  {:pre [(string? s)]
   :post [(or (nil? %) (string? %))]}
  (when (reduce (fn [acc c] (and acc (= c #?(:cljs (.toLocaleLowerCase c)
...
(defn re-quote
  ...
  regular expressions."
  [s]
  #?(:cljs (goog.string/regExpEscape s)
...
(defn- case-insensitive-contains
  ...
  (if (= needle "")
    s
    #?(:cljs (let [p (re-quote needle)]
               (when (re-find (js/RegExp. p "i") s)
                 s))
             :clj (let [p (java.util.regex.Pattern/compile
...
(defn- char-equal-ignore-case
  ...
  (when (or (= c1 c2)
            (= #?(:cljs (upper-case c1)
                        :clj (Character/toUpperCase c1)) c2)
            (= #?(:cljs (lower-case c1)
                        :clj (Character/toLowerCase c1)) c2))
    c1))
...
(defn char-at
  ...
  {:pre [(string? s) (integer? i) (< i (length s))]
   :post [#?(:cljs (string? %) :clj (instance? Character %))]}
  (.charAt s i))
...
(defn ^String strip-accents
  ...
  #?(:clj (-> s
          ...
          :cljs (let [from "ąàáäâãåăćčĉęèéëêĝĥìíïîĵłľńňòóöőôõðśșšŝťțŭùúüűûñÿýçżźž"
...
(defn ^String ascii?
  ...
  (when (reduce (fn [acc c] (and acc #?(:cljs (< (.charCodeAt c 0) 128)
                                              :clj (< (int c) 128))))
                true s)
    s))
...
(defn ^String mixed-case?
  ...
  {:pre [(string? s)]
   :post [(or (nil? %) (string? %))]}
  #?(:clj (when (and (seq (filter #(Character/isLowerCase %) s))
                     (seq (filter #(Character/isUpperCase %) s)))
            s)
          :cljs (let [caseless (set/intersection (set (seq (lower-case s)))
...
(defn longest-common-substring
  ...
  (let [rows (inc (length s1))
        cols (inc (length  s2))
        ls #?(:clj (make-array Long rows cols)
                   :cljs (let [ls (make-array rows)]
                           (dotimes [row rows]
